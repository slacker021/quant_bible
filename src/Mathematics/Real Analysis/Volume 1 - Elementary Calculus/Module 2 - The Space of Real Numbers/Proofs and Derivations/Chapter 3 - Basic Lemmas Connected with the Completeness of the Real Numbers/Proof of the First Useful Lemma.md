...
dg-publish: true
---
Let $X$ be a countable set and let $A$ be a subset of $X$. If $A$ is an empty set, it is finite and therefore at most countable. Assume $A$ is non-empty.

Since $X$ is countably infinite, there's a bijective mapping $f: \mathbb{N} \to X$. This allows the elements of $X$ to be enumerated as a sequence $x_1, x_2, \dots, x_n, \dots$, where $x_n = f(n)$.
