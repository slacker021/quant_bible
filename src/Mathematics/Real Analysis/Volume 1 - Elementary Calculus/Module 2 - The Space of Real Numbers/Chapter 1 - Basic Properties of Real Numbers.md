...
$$\begin{gather} \textbf{Proposition: Consequences of the Addition Axioms} \\ \text{1. There is only one zero in the set of real numbers.} \\ \text{2. Each element of the set of real numbers has a unique negative.} \\ \text{3. In the set } \mathbb{R}, \text{ the equation } a + x = b \text{ has the unique solution } x = b + (-a) =: b - a. \end{gather}$$
(see [[Proof of Consequences of Addition Axioms|proof]])

$$\begin{gather} \textbf{Proposition: Consequences of the Multiplication Axioms} \\ \text{1. There is only one multiplicative unit (one) in the real numbers.} \\ \text{2. For each } x \ne 0, \text{ there is only one reciprocal } x^{-1}. \\ \text{3. For } a \in \mathbb{R} \setminus \{0\}, \text{ the equation } a \cdot x = b \text{ has the unique solution } x = b \cdot a^{-1}. \end{gather}$$

(see [[Proof of Consequences of Multiplication Axioms|proof]])

$$\begin{gather} \textbf{Proposition: Consequences of the Connection Axiom} \\ \text{For any } x, y \in \mathbb{R}: \\ \text{1. } x \cdot 0 = 0 \cdot x = 0. \\ \text{2. } (x \cdot y = 0) \implies (x = 0) \lor (y = 0). \\ \text{3. } -x = (-1) \cdot x. \\ \text{4. } (-1)(-x) = x. \\ \text{5. } (-x) \cdot (-x) = x \cdot x. \end{gather}$$

(see [[Proof of the Consequences of the Connection Axioms|proof|]])

$$\begin{gather} \textbf{Proposition: Consequences of the Order Axioms} \\ \text{Strict inequality } x < y \text{ denotes } x \le y \text{ and } x \ne y. \\ \text{1. For any } x, y \in \mathbb{R}, \text{ precisely one holds: } x < y, x = y, x > y. \\ \text{2. For any } x, y, z \in \mathbb{R}: \\ (x < y) \wedge (y \le z) \implies (x < z) \\ (x \le y) \wedge (y < z) \implies (x < z) \end{gather}$$

(see [[Proof of Consequences of Order Axioms|proof]])

$$\begin{gather} \textbf{Proposition: Consequences of Order with Addition and Multiplication} \\ \text{For any } x, y, z, w \in \mathbb{R}: \\ \text{1. } (x < y) \implies (x + z) < (y + z) \\ \text{2. } (0 < x) \implies (-x < 0) \\ \text{3. } (x \le y) \wedge (z \le w) \implies (x + z) \le (y + w) \\ \text{4. } (x \le y) \wedge (z < w) \implies (x + z < y + w) \\ \text{5. } (0 < x) \wedge (0 < y) \implies (0 < xy) \\ \text{6. } (x < 0) \wedge (y < 0) \implies (0 < xy) \\ \text{7. } (x < 0) \wedge (0 < y) \implies (xy < 0) \\ \text{8. } (x < y) \wedge (0 < z) \implies (xz < yz) \\ \text{9. } (x < y) \wedge (z < 0) \implies (yz < xz) \\ \text{10. } 0 < 1 \\ \text{11. } (0 < x) \implies (0 < x^{-1}) \\ \text{12. } (0 < x) \wedge (x < y) \implies (0 < y^{-1}) \wedge (y^{-1} < x^{-1}) \end{gather}$$
(see [[Proof of Consequences of Order with Addition and Multiplication|proof]])
...
(see [proof](Proof%20of%20Consequences%20of%20Addition%20Axioms.md "null"))
