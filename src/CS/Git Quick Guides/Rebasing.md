...
>Before using `git rebase`, the user must know what branch they are on. Once the rebase is performed, Git takes all commits in the target branch against the branch then places them on top of the branch whose history is going to be altered. 
